CREATE DATABASE IF NOT EXISTS `teaching_mgt`;
...
CREATE TABLE `teacher` (
    ...
    `teach_name` VARCHAR(16) NOT NULL COMMENT '教师姓名',
...
CREATE TABLE `department` (
    ...
    `depart_name` VARCHAR(16) NOT NULL COMMENT '系部名称',
...
BEGIN
    ...
    INSERT INTO `user_login` values (NEW.`stu_no`,MD5(NEW.`stu_no`),"student","true");
END $
DELIMITER ;
...
BEGIN
    INSERT INTO `user_login` values (NEW.`teach_no`,MD5(NEW.`teach_no`),"teacher","true");
END $
DELIMITER ;
...
BEGIN
    INSERT INTO `user_login` values (NEW.`admin_no`,MD5(NEW.`admin_no`),"admin","true");
END $
DELIMITER ;
...
BEGIN
    ...
    if res=0 then
        ...
        SELECT `user_type`,`user_first` INTO u_type,u_first FROM `user_login` WHERE `user_no`=user and `user_pass`=pass;
...
INSERT INTO `admin` values("1000001", "18018592020");